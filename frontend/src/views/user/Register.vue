<template>
  <div class="main user-layout-register">
    ...
    <a-form ref="formRegister" :form="form" id="formRegister">
      ...
      <a-form-item>
        <a-input size="large" :placeholder="$t('user.login.nickname.placeholder')" v-decorator="['nickname', {rules: [{ required: true, message: $t('user.nickname.required')}], validateTrigger: ['change', 'blur'] }]">
        </a-input>
      </a-form-item>
        ...
        (err, values) => {
          if (!err) {
            ...
            $notification['success']({
                message: '提示',
                description: '验证码获取成功，您的验证码为：' + (Math.floor(Math.random() * (999999 - 100000 + 1)) + 100000),
                duration: 8
              })
          }
        }
