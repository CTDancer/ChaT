...
export default {
  ...
  methods: {
    fetchPosts () {
      ...
      this.loading = true
      axios.get('http://127.0.0.1:8000/api/hole', { params: { division_id: 5, index: this.index, currentname: localStorage.getItem('nickname') } })
        .then(res => {
    ...
    loadMore () {
      ...
      this.index += 1
      axios.get('http://127.0.0.1:8000/api/hole', { params: { division_id: 5, index: this.index } })
        .then(res => {
